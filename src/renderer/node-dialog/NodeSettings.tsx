...
function ModelOptions({ current, models }: { readonly current: string; readonly models: readonly ModelOption[] }) {
  const available = models.some((model) => model.id === current);
  return <>{available ? null : <option value={current}>Unavailable — {current}</option>}{models.length === 0 && available ? <option value={current}>{current}</option> : models.map((model) => <option key={model.id} value={model.id}>{model.name}</option>)}</>;
}

function RuntimeSettings({ node, harnesses, models }: {
  ...
  return (
    <section data-section="runtime">
      ...
      <div className="node-dialog-field-grid">
        <Field label="HARNESS"><select aria-label="Harness" disabled={harnessChanging} value={node.harnessId} onChange={(event) => void onHarnessChange(event.target.value)}>{harnesses.length === 0 ? <option value={node.harnessId}>{node.harnessId}</option> : harnesses.map((harness) => <option key={harness.id} value={harness.id} disabled={!harness.status.connected}>{harness.name} · {harness.status.connected ? "connected" : "offline"}</option>)}</select></Field>
        <Field label="MODEL"><div className="node-model-control"><select aria-label="Model" value={node.modelId} onChange={(event) => updateNode(node.id, { modelId: event.target.value })}><ModelOptions current={node.modelId} models={models} /></select><button type="button" aria-label="Refresh models" title="Refresh models" onClick={() => void loadHarnessModels(node.harnessId)}><RefreshCw size={15} /></button></div></Field>
        <Field label="ACTIVATION"><select value={node.activation} onChange={(event) => updateNode(node.id, { activation: event.target.value === "any" ? "any" : "all" })}><option value="all">All inputs</option><option value="any">Any input</option></select></Field>
        <Field label="MAX VISITS"><input type="number" min={1} value={node.maxVisits} onChange={(event) => updateNode(node.id, { maxVisits: Number(event.target.value) })} /></Field>
      </div>
    </section>
  );
}

function AgentSettings({ node, harnesses, models }: {
  readonly node: Extract<GraphNode, { readonly kind: "agent" | "decision" }>;
  readonly harnesses: readonly HarnessStatus[];
  readonly models: readonly ModelOption[];
}) {
  ...
  return <>
    ...
    <RuntimeSettings node={node} harnesses={harnesses} models={models} />
...
export function NodeSettings({ graph, node, harnesses, harnessModels }: {
  ...
  return <div className="node-dialog-panel-body node-settings-body">
    ...
    {node.kind === "agent" || node.kind === "decision" ? <AgentSettings node={node} harnesses={harnesses} models={harnessModels[node.harnessId] ?? []} /> : null}
